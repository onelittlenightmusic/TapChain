...
import org.tapchain.ActorManager.IPathEdit;
import org.tapchain.ActorManager.IPieceEdit;
...
public class TapChainEdit implements IPieceEdit, IPathEdit, IControlCallback {
	...
	TreeMap<IPiece,IPieceView> dictPiece = new TreeMap<IPiece, IPieceView>();
	...
	protected TapChainEdit() {
		...
		editorManager.setEditor(this).setFactory(userFactory).createChain(20);
		userManager.setEditor(this).setFactory(userFactory).createChain(50).get().setAutoEnd(false);
	...
	public void freezeToggle() {
		getUserManager().get().ctrl.Toggle();
	}
	...
	public static class AddPiece extends Actor {
		AddPiece() {
			super();
			boost();
			setInPackType(PackType.PASSTHRU, ChainPiece.Input.FIRST);
		}
	...
	public IPieceView setView(final IPiece cp2, final Blueprint bp) {
		...
		if(cp2 instanceof ChainPiece) {
			...
			c.setStatusHandler(new StatusHandler() {
				@Override
...
//			return 
	...
	*/
	...
	public void Compile() {
		userManager.get().getOperator().reset();
		userManager.get().Set(new IControlCallback() {
			public boolean callback() {
				kickDraw();
				return true;
			}
	...
	public void start() {
		...
		userManager.get().getOperator().start();
		return;
	}
	...
	void reformTo(Actor bp, final boolean UpDown, final WorldPoint pt) {
		...
		if (pt_ != null) {
			...
			for (ChainOutConnector _cp : bp.getOutPack(PackType.FAMILY)) {
...
//				return ConnectType.RELEASING;
			...
			if (v1.getInteraction().checkIn(v1, v2)) {
				...
				getUserManager().refreshView(bp, target);
